...
static int hnat_probe(struct platform_device *pdev)
{
	...
	if (err < 0)
		strscpy(hnat_priv->wan, "eth1", IFNAMSIZ);
	else
	...
	err = of_property_read_string(np, "hnat-bond-wan", &name);
	if (err < 0 || !name || !name[0])
		strscpy(hnat_priv->bond_wan, "bond1", IFNAMSIZ);
	else
		strncpy(hnat_priv->bond_wan, (char *)name, IFNAMSIZ - 1);
	dev_info(&pdev->dev, "bond_wan = %s\n", hnat_priv->bond_wan);

	err = of_property_read_string(np, "hnat-bond-lan", &name);
	if (err < 0 || !name || !name[0])
		strscpy(hnat_priv->bond_lan, "bond0", IFNAMSIZ);
	else
		strncpy(hnat_priv->bond_lan, (char *)name, IFNAMSIZ - 1);
	dev_info(&pdev->dev, "bond_lan = %s\n", hnat_priv->bond_lan);
