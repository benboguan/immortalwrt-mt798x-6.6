...
int dpi_using = 0;
u_int64_t dpi_limit_bytes = 1500 * 30; // 45000 bytes
...
static int hnat_set_usage(int level)
{
	debug_level = level;
...
static  int hnat_cpu_reason(int cpu_reason)
{
...
static int entry_set_usage(int level)
{
	debug_level = level;
...
static int entry_set_state(int state)
{
...
static int wrapped_ppe0_entry_detail(int index)
{
	entry_detail(0, index);
	return 0;
}

static int wrapped_ppe1_entry_detail(int index)
{
	entry_detail(1, index);
	return 0;
}

static int wrapped_ppe2_entry_detail(int index)
{
...
static int wrapped_ppe0_entry_delete(int index)
{
	entry_delete(0, index);
	return 0;
}

static int wrapped_ppe1_entry_delete(int index)
{
...
static int delete_entry_by_mac(char *mac_str)
{
	char mac[6];
...
static int delete_entry_by_ip(bool is_ipv4, char *str)
{
...
static int cr_set_usage(int level)
{
	...
	pr_info("              12    0~1        Set hnat disable/enable guest (rax1/rai1/ra1)\n");
	pr_info("              13    0~1        Set hnat disable/enable dscp setting\n");
	pr_info("              14    0~1        Set hnat macvlan support mode\n");
...
static int binding_threshold(int threshold)
{
	...
	if (threshold < 0) {
		pr_warn("Binding Threshold = %d < 0, clamped to 0\n", threshold);
		threshold = 0;
	}
	if (threshold > 65535) {
		pr_warn("Binding Threshold = %d > 65535, clamped to 65535\n", threshold);
		threshold = 65535;
	}
	...
	for (i = 0; i < CFG_PPE_NUM; i++)
		cr_set_field(hnat_priv->ppe_base[i] + PPE_BNDR, BIND_RATE, threshold);
...
static int set_ipv6_toggle(int toggle)
{
	struct mtk_hnat *h = hnat_priv;
...
static int set_dscp_toggle(int toggle)
{
	struct mtk_hnat *h = hnat_priv;
...
static int set_macvlan_support(int toggle)
{
	struct mtk_hnat *h = hnat_priv;
...
static void mtk_ppe_dev_hook(const char *name, int toggle)
{
...
static int set_guest_toggle(int toggle)
{
	struct mtk_hnat *h = hnat_priv;
...
static const debugfs_write_func cr_set_func[] = {
	...
	[11] = set_ipv6_toggle,   [12] = set_guest_toggle,
	[13] = set_dscp_toggle,  [14] = set_macvlan_support,
};
		...
		if (IS_IPV4_HNAPT(entry)) {
			...
			*((u16 *)&h_dest[4]) =
			...
			seq_printf(m, "addr=0x%p|ppe=%d|index=%d state=%s\n",
				   entry, ppe_id, ei(entry, end), es(entry));
		entry++;
		entry_index++;
	}
...
static int cpu_reason_read(struct seq_file *m, void *private)
{
	int i;
...
static void dbg_dump_entry(struct seq_file *m, struct foe_entry *entry,
			   uint32_t index)
{
	__be32 saddr, daddr, nsaddr, ndaddr;
				      ...
				      size_t count, loff_t *data)
{
	...
	if (!strncmp(buf, "ipv4 add", 8)) {
		...
		read_lock_bh(&pmcast->mcast_lock);
		list_for_each_entry(m, &h->mcast_blist_list, list) {
			if (!m->is_ipv4 && ipv6_addr_equal(&m->ipv6, &ipv6)) {
				pr_info("this ip already added.\n");
				read_unlock_bh(&pmcast->mcast_lock);
				return len;
			}
		...
		write_lock_bh(&pmcast->mcast_lock);
		list_for_each_entry_safe(m, next, &h->mcast_blist_list, list) {
			if (!m->is_ipv4 && ipv6_addr_equal(&ipv6, &m->ipv6)) {
				list_del(&m->list);
				kfree(m);
				pr_info("ipv6 del: %s\n", v6_str);
				write_unlock_bh(&pmcast->mcast_lock);
				       ...
				       size_t count, loff_t *data)
{
	...
	char *p_buf, *token;
	u8 proto_list[16] = {0};
	//u32 prot_regs[4] = {0}; // unused variable 'prot_regs'
...
static int parse_vids(char *ids)
{
	int ret, i = 0;
...
int hnat_init_debugfs(struct mtk_hnat *h)
{
	...
	debugfs_create_file("external_interface", 0444, root, h,
			    &hnat_ext_fops);
	debugfs_create_file("whnat_interface", 0644, root, h,
			    &hnat_whnat_fops);
	debugfs_create_file("cpu_reason", 0644, root, h,
			    &cpu_reason_fops);
	debugfs_create_file("hnat_entry", 0644, root, h,
			    &hnat_entry_fops);
	...
	debugfs_create_file("mcast_hook_toggle", 0644, root, h,
			    &hnat_mcast_hook_toggle_fops);
	debugfs_create_file("mcast_blist", 0644, root, h,
			    &hnat_mcast_blist_fops);
	debugfs_create_file("mcast_member", 0444, root, h,
			    &hnat_mcast_member_fops);
	debugfs_create_file("dpi_using", 0644, root, h,
			    &hnat_dpi_using_fops);
	debugfs_create_file("dpi_limit_bytes", 0644, root, h,
			    &hnat_dpi_limit_fops);
#if 1 /* ASUS: skip specific VID */
	debugfs_create_file("skip_vid", 0644, root, h,
			    &hnat_skip_vid_fops);
#endif
	debugfs_create_file("mape_toggle", 0644, root, h,
			    &hnat_mape_toggle_fops);
	debugfs_create_file("qos_toggle", 0644, root, h,
			    &hnat_qos_toggle_fops);
	debugfs_create_file("hnat_version", 0444, root, h,
			    &hnat_version_fops);
	debugfs_create_file("hnat_ppd_if", 0644, root, h,
			    &hnat_ppd_if_fops);
	debugfs_create_file("static_entry", 0644, root, h,
			    &hnat_static_fops);
	debugfs_create_file("tnl_toggle", 0644, root, h,
			    &hnat_tnl_toggle_fops);
	debugfs_create_file("xlat_toggle", 0644, root, h,
			    &hnat_xlat_toggle_fops);
	debugfs_create_file("xlat_cfg", 0644, root, h,
			    &hnat_xlat_cfg_fops);
	debugfs_create_file("l2br_toggle", 0644, root, h,
			    &hnat_l2br_toggle_fops);
	debugfs_create_file("l4s_toggle", 0644, root, h,
