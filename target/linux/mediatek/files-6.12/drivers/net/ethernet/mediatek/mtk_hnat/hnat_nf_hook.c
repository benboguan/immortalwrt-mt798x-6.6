...
static struct net_device *hnat_get_up_dev(const char *name)
{
...
{
	struct net_device *rx = NULL, *tx = NULL;
	struct net_device *br, *dev;
	struct list_head *pos;
	const char * const rx_names[] = {
		hnat_priv->ppd, hnat_priv->lan2,  /* "hnat" */
		hnat_priv->bond_lan,  hnat_priv->lan,   /* "bond0", "eth0" */
		hnat_priv->bond_wan,  hnat_priv->wan,   /* "bond1", "eth1" */
	};
	int i;

	/* RX：up + carrier_ok */
	for (i = 0; i < ARRAY_SIZE(rx_names) && !rx; i++)
		rx = hnat_get_up_dev(rx_names[i]);

	/* TX：br-lan, bond */
	if ((br = dev_get_by_name(&init_net, "br-lan"))) {
		netdev_for_each_lower_dev(br, dev, pos) {
			if (!(dev->flags & IFF_UP) || !netif_carrier_ok(dev))
				continue;
			...
			if (IS_BOND(dev)) {
				if (tx)
					dev_put(tx);
				tx = dev_get_by_name(&init_net, dev->name);
				break;
			}
			if (!tx)
				tx = dev_get_by_name(&init_net, dev->name);
	...
	/* Single-arm routing */
	if (!rx && tx)
		rx = dev_get_by_name(&init_net, tx->name);
	if (!tx && rx)
		tx = dev_get_by_name(&init_net, rx->name);
	...
	hnat_priv->g_ppdev = rx;
	ppd_dev = tx;

	pr_info("%s : rx=%s tx=%s\n", __func__,
		rx ? rx->name : "(null)", tx ? tx->name : "(null)");
}
				...
				struct sk_buff *skb, int val)
{
	if (IS_WHNAT(state->in) && FROM_WED(skb)) {
		return;
	} else if (IS_WHNAT(state->in) || IS_LAN(state->in)) {
				 ...
				 u32 state)
{
	...
	memcpy(&foe->data, &entry->data,
	       PPE_ENTRY_SIZE - sizeof(foe->info_blk1));
	/* We must ensure all info has been updated before set to hw */
	wmb();
...
hnat_entry_bind:
	...
	if ((IS_HQOS_MODE) && (dscp!=0) && (hnat_priv->dscp_en))
		qid = (dscp >> 2) & (MTK_QDMA_QUEUE_MASK);
...
int mtk_hnat_eth_hook_tx(struct sk_buff *skb, struct net_device *dev)
{
	...
	if ((skb_hnat_reason(skb) != HIT_UNBIND_RATE_REACH) || skb_hnat_alg(skb) ||
	    !is_magic_tag_valid(skb) || !is_hnat_info_filled(skb))
		return NF_ACCEPT;
	...
	if (netdev_uses_dsa(dev)) {
		...
		if (IS_DSA_TAG_PROTO_8021Q(dp)) {
			...
			if (skb_headlen(skb) < 16)
				return NF_ACCEPT;

			dsa_ptr = ((const u8 *)skb->data) + 12;
			dsa_tag = dsa_ptr[1] & 0x3F;
	 ...
	 * prevent racing with hardware accesses.
	 */
	memcpy(&hw_entry->data, &entry.data,
			PPE_ENTRY_SIZE - sizeof(hw_entry->info_blk1));
	/* We must ensure all info has been updated before set to hw */
	wmb();
