...
struct mtk_hnat {
	...
	char lan2[IFNAMSIZ];
	char ppd[IFNAMSIZ];
	char bond_wan[IFNAMSIZ];
	char bond_lan[IFNAMSIZ];
	u16 lvid;
	u16 wvid;
...
#define IS_WAN(dev) ((!strncmp((dev)->name, WAN_DEV_NAME, strlen(WAN_DEV_NAME))) ||		\
		 ((hnat_priv->macvlan_support) && (!strncmp((dev)->name, "macvlan", 7))) ||		\
		 (!strncmp((dev)->name, hnat_priv->bond_wan, strlen(hnat_priv->bond_wan))))
#define IS_LAN_GRP(dev) (IS_LAN(dev) | IS_LAN2(dev))
#define IS_LAN(dev)								\
	(!strncmp(dev->name, LAN_DEV_NAME, strlen(LAN_DEV_NAME)) ||		\
	 (!strncmp(dev->name, hnat_priv->bond_lan, strlen(hnat_priv->bond_lan)) /*IS_BOND(dev)*/))
#define IS_LAN2(dev)								\
	(!strncmp(dev->name, LAN2_DEV_NAME, strlen(LAN2_DEV_NAME)) ||		\
	 (IS_BOND(dev) && strncmp(dev->name, hnat_priv->bond_wan, strlen(hnat_priv->bond_wan))))
