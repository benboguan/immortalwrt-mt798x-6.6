...
INT ht_support_bw_by_channel_boundary(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, IE_LISTS *peer_ie_list)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_DEBUG,
		"Channel = %d, supported_bw = %d, extcha = %d, PeerSupChlLen = %d\n",
		wdev->channel,
		supported_bw,
		ext_cha,
	...
	if ((wlan_config_get_ch_band(wdev) == CMD_CH_BAND_24G)
		&& (peer_ie_list->SupportedChlLen >= 2)) {
		UCHAR i;
		UCHAR peer_sup_chl_min_2g = 1;
		UCHAR peer_sup_chl_max_2g = 14;

		/* Get Min/Max 2.4G supported channels */
		for (i = 0; i < ARRAY_SIZE(peer_ie_list->SupportedChl) &&
				i < peer_ie_list->SupportedChlLen; i += 2) {
...
INT ht_mode_adjust(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *pEntry, HT_CAPABILITY_IE *peer_ht_cap)
{
	...
	pEntry->cap.ch_bw.ht_support_ch_width_set = peer_ht_cap->HtCapInfo.ChannelWidth;

	MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO,
		"HT BW from assoc = %d\n", pEntry->cap.ch_bw.ht_support_ch_width_set);
...
UINT16 ht_max_mpdu_size[3] = {3839u, 7935u, 11454u};
...
VOID set_sta_ht_cap(RTMP_ADAPTER *pAd, MAC_TABLE_ENTRY *entry, HT_CAPABILITY_IE *ht_ie)
{
	...
	BOOLEAN set_amsdu_by_ht_mode = TRUE;
...
#ifdef DOT11_EHT_BE
	if (entry->mlo.mlo_en && entry->AMsduSize)
		set_amsdu_by_ht_mode = FALSE;
#endif /* DOT11_EHT_BE */
	if (set_amsdu_by_ht_mode == TRUE)
	...
	IN struct wifi_dev *wdev)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO,
		"HT_mode(%d), ExtOffset(%d), MCS(%d), BW(%d), STBC(%d), SHORTGI(%d)\n",
...
VOID RTMPSetIndividualHT(RTMP_ADAPTER *pAd, UCHAR apidx)
{
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_DEBUG, "apidx=%d\n", apidx);
	...
	do {
		...
		if (apidx >= MIN_NET_DEVICE_FOR_APCLI) {
			...
			if (idx < MAX_APCLI_NUM) {
				...
				MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "invalid idx(%d)\n", idx);
				return;
			}
		}
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
			...
			if (apidx >= MIN_NET_DEVICE_FOR_WDS) {
				...
				if (idx < MAX_WDS_ENTRY) {
					...
					MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "invalid apidx(%d)\n", apidx);
					return;
				}
			}
			...
			if ((apidx < pAd->ApCfg.BssidNum) && (VALID_MBSS(pAd, (int)apidx))) {
	...
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO, "Desired MCS = %d\n", DesiredMcs);

	/* Check the validity of MCS */
	if ((TxStream == 1) && ((DesiredMcs >= MCS_8) && (DesiredMcs <= MCS_15))) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_WARN,
			"MCS(%d) is invalid in 1S, reset it as MCS_7\n", DesiredMcs);
		DesiredMcs = MCS_7;
	}
	...
	if ((cfg_ht_bw == HT_BW_20) && (DesiredMcs == MCS_32)) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_WARN,
			"MCS_32 is only supported in 40-MHz, reset it as MCS_0\n");
		DesiredMcs = MCS_0;
	}
	...
	if ((pAd->OpMode == OPMODE_STA) &&
		...
		*/
		if (pAd->CommonCfg.HT_DisallowTKIP && IS_INVALID_HT_SECURITY(encrypt_mode)) {
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_WARN,
				"Use legacy rate in WEP/TKIP encryption mode (apidx=%d)\n",
				apidx);
			return;
		}

	if (pAd->CommonCfg.HT_Disable) {
		MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO, "HT is disabled\n");
		return;
	}

	if (!IS_CIPHER_WEP(encrypt_mode))
...
#endif /* DOT11_VHT_AC */
	MTWF_DBG(NULL, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_INFO,
			"DesiredHtPhyInfo HtEn = %d, VhtEn = %d, vht_bw = %d\n",
...
UINT32 starec_ht_feature_decision(
	struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry, UINT64 *feature)
{
	...
	if ((cap->ppdu.tx_amsdu_support == TRUE) && amsdu_en
		/*
...
#endif
		ba_info->TxAmsduBitmap = 0xffff;
	} else {
		ba_info->TxAmsduBitmap = 0x0;
	}
	...
	IN ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	sta = entry_get(pAd, Wcid);
	pStaCfg = GetStaCfgByWdev(pAd, sta->wdev);
	ASSERT(pStaCfg);
	if (pStaCfg == NULL) {
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "StaCfg NULL!! wcid %d entryType %d\n", Wcid, sta->EntryType);
		if (sta->wdev)
			MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_DEBUG, "wdev %s, idx %d\n", sta->wdev->if_dev->name, sta->wdev->wdev_idx);
		return FALSE;
	}
	...
	struct common_ies *cmm_ies)
{
	...
	sta = entry_get(ad, wcid);
	sta_cfg = GetStaCfgByWdev(ad, sta->wdev);
	ASSERT(sta_cfg);
	if (sta_cfg == NULL) {
		MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "StaCfg NULL!! wcid %d entryType %d\n", wcid, sta->EntryType);
		if (sta->wdev)
			MTWF_DBG(ad, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_DEBUG, "wdev %s, idx %d\n", sta->wdev->if_dev->name, sta->wdev->wdev_idx);
		return FALSE;
	}
...
static INT build_ht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UINT16 ReptMacTabWCID, UCHAR *buf)
{
	...
	if (wdev == NULL) {/*ALPS05330340*/
		MTWF_DBG(pAd, DBG_CAT_PROTO, CATPROTO_HT, DBG_LVL_ERROR, "wdev is null\n");
		return 0;
	}
...
#endif /* CONFIG_STA_SUPPORT */
#ifdef TXBF_SUPPORT
	if (wdev && (bf_is_support(wdev) == FALSE)) {/*ALPS05330254*/
		UCHAR ucEBfCap;
		...
		if (cap->FlgHwTxBfCap & TXBF_AID_HW_LIMIT) {
			...
			if ((ReptMacTabWCID != 0) && (ReptMacTabWCID < WTBL_MAX_NUM(pAd)) &&
					(pAd->fgClonedStaWithBfeeSelected == FALSE)) {
				MAC_TABLE_ENTRY *pEntry = entry_get(pAd, ReptMacTabWCID);
				...
				if ((pEntry) && (bf_is_support(pEntry->wdev) == TRUE)) {
