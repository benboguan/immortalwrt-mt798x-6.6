...
static VOID EFUSE_IO_READ32(
	struct _RTMP_ADAPTER *pAd, UINT32 reg, UINT32 *value)
{
	RTMP_IO_READ32(pAd->hdev_ctrl, reg, value);
}


static VOID EFUSE_IO_WRITE32(
	struct _RTMP_ADAPTER *pAd, UINT32 reg, UINT32 value)
{
	RTMP_IO_WRITE32(pAd->hdev_ctrl, reg, value);
}
	...
	OUT	USHORT *pData)
{
	...
	UINT32	data, eFuseDataBuffer[4] = {0};
	UINT32 efuse_ctrl_reg = EFUSE_CTRL;
#ifdef MT_MAC
	...
	IN	USHORT *pData)
{
	...
	USHORT	LogicalAddress[1] = {0}, BlkNum = 0xffff;
	UCHAR	EFSROM_AOUT;
	...
	EFSROM_AOUT = eFuseReadRegisters(pAd, tmpOffset, 2, NULL);
	...
	if (EFSROM_AOUT == 0x3f) {
		...
		for (i = StartBlock; i >= EndBlock; i -= 2) {
			...
			EfusePhysicalReadRegisters(pAd, i, 2, &LogicalAddress[0]);
	...
	if (!bWriteSuccess)
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR, "Efsue Write Failed!!\n");
...
INT set_eFuseGetFreeBlockCount_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_PRINT("set_eFuseGetFreeBlockCount_Proc %d\n", PD_GET_USE_EFUSE(pAd->physical_dev));
...
INT set_eFusedump_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (PD_GET_USE_EFUSE(pAd->physical_dev) == FALSE)
		return FALSE;
	...
	for (i = 0; i < cap->EFUSE_USAGE_MAP_END / 2; i++) {
		...
		MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "%04x ", InBuf[2]);
	}
...
INT	set_eFuseLoadFromBin_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_INFO, "FileName=%s\n", src);
...
BOOLEAN rtmp_ee_efuse_read16(RTMP_ADAPTER *pAd, UINT32 Offset, UINT16 *pValue)
{
		...
		if (memcmp(ptr, all_ff, EFUSE_BLOCK_SIZE)) {
			...
			for (index = 0; index < EFUSE_BLOCK_SIZE; index++) {
				...
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_DEBUG,
					"index 0x%04x: ", index);
				MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_DEBUG,
					"original block value=0x%04x, write value=0x%04x\n",
							...
							PUINT EfuseFreeBlock)
{
	...
	*EfuseFreeBlock = 0;
	...
	if (ops->get_efuse_free_blk_bnum_v2) {
		*EfuseFreeBlock = ops->get_efuse_free_blk_bnum_v2(pAd, PD_GET_EFUSE_BLK_INFO_PTR(pAd->physical_dev));
	} else if (ops->get_efuse_free_blk_bnum) {
		/* blk section = 0x0 implies checking D-Die part */
		*EfuseFreeBlock = ops->get_efuse_free_blk_bnum(pAd, die_idex);
	} else {
		USHORT i = 0, StartBlock = 0, EndBlock = 0;
		USHORT	LogicalAddress[1] = {0};
...
INT eFuse_init(RTMP_ADAPTER *pAd)
{
	...
	if (EfuseFreeBlock >= cap->EFUSE_RESERVED_SIZE) {
		...
		PD_SET_USE_EFUSE(pAd->physical_dev, FALSE);
...
INT efuse_probe(struct _RTMP_ADAPTER *ad)
{
	UINT32 eFuseCtrl = 0, ctrl_reg;

	PD_SET_USE_EFUSE(ad->physical_dev, FALSE);
	ctrl_reg = MT_EE_CTRL;
	EFUSE_IO_READ32(ad, ctrl_reg, &eFuseCtrl);
	MTWF_PRINT("%s: efuse = %x\n", __func__, eFuseCtrl);

	if ((eFuseCtrl & 0x40000000) == 0x40000000)
		PD_SET_USE_EFUSE(ad->physical_dev, FALSE);
	else
		PD_SET_USE_EFUSE(ad->physical_dev, TRUE);
...
{
	UINT16 efuse_val = 0, efuse_chip_id = 0;
	UINT free_blk = 0;
	UINT i;
	...
	if (cap->efuse_free_block_ver == EFUSE_FREE_BLOCK_FORMAT_V2) {
		rtmp_ee_efuse_read16(pAd, 0, &efuse_chip_id);
		efuse_chip_id = cpu2le16 (efuse_chip_id);
		if ((pAd->ChipID & 0x0000ffff) != efuse_chip_id) {
			MTWF_DBG(pAd, DBG_CAT_HW, CATHW_EEPROM, DBG_LVL_ERROR,
				"efuse is invalid chip id: %04x, free_blk:%u\n",
				efuse_chip_id, free_blk);
			return;
		}
	}
	...
	for (i = 0; i < cap->EEPROM_DEFAULT_BIN_SIZE; i += 2) {
		...
		NdisMoveMemory(&ph_dev->EEPROMImage[i], &efuse_val, 2);
	}
...
INT set_eFuseBufferModeWriteBack_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_INFO,
		"Write EEPROM buffer back to eFuse\n");
...
{
	MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_INFO, "Write EEPROM buffer back to BIN\n");
...
INT set_eFuseWrite_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT32 offset;
	USHORT data;
	CHAR *value;
	USHORT origin_data = 0;

	if (strlen(arg) != 7) { /* offset:data, acceptable format 0300:45 length 7 */
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_ERROR,
		"Acceptable format is offset:data. Ex: 0300:45\n");
		return false;
	}
	...
	offset = os_str_tol(value, 0, 16);
	if (offset != 0x300) {
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_ERROR,
		"Only offset = 0x300 is available now.\n");
		return FALSE;
	}

	value = rstrtok(NULL, ":");
	if (value == NULL || strlen(value) != 2) {
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_ERROR, "Value length is 2.\n");
		return FALSE;
	}

	data = os_str_tol(value, 0, 16);
	if (data > 0xFF) {
		MTWF_DBG(pAd, DBG_CAT_TEST, CATTEST_ATE, DBG_LVL_ERROR,
		"Can only set a single byte value.\n");
		return FALSE;
	}

	rtmp_ee_efuse_read16(pAd, offset, &origin_data);
	origin_data &= 0xff00;
	data += origin_data;
	rtmp_ee_efuse_write16(pAd, offset, data);
...
INT show_efuseinfo_proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (ops->get_sku_decision)
		sku = ops->get_sku_decision(pAd);

	if (sku == MT7990_ADIE_677 || sku == MT7990_ADIE_577) {
		/* need to add adie1 size (3k) for efuse info dump */
		length = length + (1024 * 3);
	}
#endif

	for (; offset < length; offset += EFUSE_BLOCK_SIZE) {
		MTWF_PRINT("offset 0x%04x: ", offset);
		...
		for (index = 0; index < EFUSE_BLOCK_SIZE; index += 2) {
			...
			MTWF_PRINT("%02x %02x ", value & 0xFF, (value >> 8) & 0xFF);
		}
