				  ...
				  union iwreq_data *wrqu, char *extra)
{
	...
	if (ret != NDIS_STATUS_SUCCESS || !IW_IS_GET(info->cmd) || extra == NULL)
		return ret;
	...
	if (info->cmd < SIOCIWFIRSTPRIV) {
		...
		data_len = (UINT16)(wrqu->data.length * standard_descr->token_size);
...
INT rt28xx_ap_ioctl(void *net_dev_obj, void *data_obj, int cmd) /* snowpin for ap/sta */
{
	...
	if (cmd == SIOCGIWNAME) {
		MTWF_DBG(NULL, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_INFO, "IOCTL::SIOCGIWNAME\n");
	...
	break;
	case RTPRIV_IOCTL_TESTENGINE: {
	    RTMP_COM_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_TESTENGINE, 0, wrqin->ifr_name, 0);
	...
	case RTPRIV_IOCTL_SET_FT_PARAM:
		{
			...
			if (erq->length != sizeof(ap_11r_params)) {
				Status = 0;
			...
			if ((ap_11r_params.nas_id_len < 0) ||
			    (ap_11r_params.nas_id_len > sizeof(ap_11r_params.nas_identifier))) {
				Status = -EINVAL;
				break;
			}
			Status = RTMP_AP_IoctlHandle(pAd, wrq, CMD_RTPRIV_IOCTL_SET_FT_PARAM, 0,
				(void *)&ap_11r_params, sizeof(ap_11r_params));
		}
		break;
#endif
	...
	case SIOCGIWTXPOW:/*get transmit power (dBm) */
	{
		...
		MTWF_DBG(NULL, DBG_CAT_CFG, CATCFG_CMD, DBG_LVL_INFO, "power = %d\n", powerval);
	...
	case RT_PRIV_IOCTL_EXT: {
		...
		MTWF_PRINT("\n\n**************************** WARNING ****************************\n"
			"iwpriv/ioctl will be phase out after 2022/4/E [RT_PRIV_IOCTL=SIOCIWFI RSTPRIV + 0x01, subcmd=%d].\n"
			"If you see this message, it means the command needs to be replaced with correct NL80211_CMD_ID.\n"
			"caller pid: %d, caller name:%.15s\n\n", subcmd, current->pid, current->comm);
#endif
...
#ifdef RT_CFG80211_SUPPORT
			MTWF_PRINT("\n\n*********************** WARNING ***********************\n"
				"iwpriv/ioctl will be phase out after 2022/4/E [RTPRIV_IOCTL_SET=SIOCIWFIRSTPRIV + 0x02].\n"
				"If you see this message, it means the command needs to be replaced with correct NL80211_CMD_ID.\n"
...
#ifdef RT_CFG80211_SUPPORT
			MTWF_PRINT("\n\n*********************** WARNING ***********************\n"
				"iwpriv/ioctl will be phase out after 2022/4/E [RTPRIV_IOCTL_SHOW=SIOCIWFIRSTPRIV + 0x11].\n"
				"If you see this message, it means the command needs to be be replaced with correct NL80211_CMD_ID.\n"
				"caller pid: %d, caller name:%.15s\n\n", current->pid, current->comm);
