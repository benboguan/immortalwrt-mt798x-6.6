...
#if defined(MTK_FE_RESET_RECOVER) || defined(WF_RESET_SUPPORT)
...
#define AP_DRIVER_VERSION	"8.2.1.4"
#endif
...
#define RT_MOD_INC_USE_COUNT() \
	do {\
		if (!try_module_get(THIS_MODULE)) { \
			MTWF_DBG(NULL, DBG_CAT_INIT, CATINIT_INTF, DBG_LVL_ERROR, \
				"cannot reserve module\n"); \
			return -1; \
		}	\
	} while (0)
...
#define RT_GET_OS_PID(_x, _y)		do {mt_rcu_read_lock(); _x = current->thread_pid; mt_rcu_read_unlock(); } while (0)
#else
#define RT_GET_OS_PID(_x, _y)		do {mt_rcu_read_lock(); _x = current->pids[PIDTYPE_PID].pid; mt_rcu_read_unlock(); } while (0)
#endif
...
#if (KERNEL_VERSION(3, 5, 0) <= LINUX_VERSION_CODE) && defined(CONFIG_MT7621_ASIC)
...
#define OS_PAGE_REF(_p) atomic_read(&_p->_refcount)
#else
...
#define DEV_ALLOC_FRAG(_Pkt, _length)		\
	do {\
		...
		if (_Pkt != NULL) {	\
			MEM_DBG_PKT_ALLOC_INC_SIZE(_Pkt,_length);		\
		};	\
	} while (0)
...
#define DEV_SKB_PTR_ADJUST(_Pkt, _length, _buf_ptr, _buf_pad)		\
	do {\
		if (_Pkt != NULL) {						\
			...
			skb_reserve((_Pkt), (SKB_BUF_HEADROOM_RSV) + _buf_pad);	\
 ...
 *	64 bit operand
 ***********************************************************************************/
#define mod_64bit(_a, _b) do_div(_a, _b)
...
#if (LINUX_VERSION_CODE  >= KERNEL_VERSION(3, 8, 0))
#define os_random32 get_random_u32
#else /* LINUX_VERSION_CODE >= KERNEL_VERSION(3,8,0) */
#define os_random32 random32
...
#define RTUSB_CONTROL_MSG(pUsb_Dev, uEndpointAddress, Request, RequestType, Value, Index, tmpBuf, TransferBufferLength, timeout, ret)	\
	do {	\
		if ((RequestType == DEVICE_VENDOR_REQUEST_OUT) || (RequestType == DEVICE_CLASS_REQUEST_OUT))	\
			...
			MTWF_DBG(NULL, DBG_CAT_HIF, CATHIF_USB, DBG_LVL_ERROR, "vendor request direction is failed\n");	\
			ret = -1;	\
		}	\
	} while (0)
...
#define os_idr_for_each_entry(_idrm, _entry, _id) \
	idr_for_each_entry(&(_idrm)->id, _entry, _id)
#define os_idr_for_each_entry_continue(_idrm, _entry, _id) \
	idr_for_each_entry_continue(&(_idrm)->id, _entry, _id)
