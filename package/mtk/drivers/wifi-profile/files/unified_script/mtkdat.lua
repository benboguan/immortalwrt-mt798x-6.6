...
--local map_help = require "map_helper"
local mtkdat = {}
...
local function get_factory_mtdblock()
    local f = io.open("/proc/mtd", "r")
    if not f then return nil end
    for line in f:lines() do
        local mtd, name = line:match("^(mtd%d+):.*\"(Factory)\"$")
        if mtd and name == "Factory" then
            f:close()
            local num = mtd:match("mtd(%d+)")
            return num and ("/dev/mtdblock" .. num) or nil
        end
    end
    f:close()
    return nil
...
-- 从 Factory 指定偏移读取 6 字节，返回 MAC 字符串 (xx:xx:xx:xx:xx:xx)
local function get_mac_from_factory(offset)
    local mtdblock = get_factory_mtdblock()
    if not mtdblock then return nil end
    local cmd = string.format("dd if=%s bs=1 skip=%d count=6 2>/dev/null | od -An -tx1 -v | tr -d ' \n'", mtdblock, offset)
    local handle = io.popen(cmd)
    local hex = handle:read("*a")
    handle:close()
...
-- 检查 MAC 是否为无效前缀 (00:0c 或 0c:43:26)
local function is_mac_invalid(mac)
    if not mac then return true end
    return mac:lower():match("^00:0c") or mac:lower():match("^0c:43:26")
end

-- 使用 mmc 生成基 MAC（相当于 LAN MAC）
local mmc_base_mac = nil
local function get_mmc_base_mac()
    if mmc_base_mac then return mmc_base_mac end
    local cmd = 'sh -c ". /lib/functions/system.sh; macaddr_generate_from_mmc_cid mmcblk0"'
    local handle = io.popen(cmd)
    ...
    error("Failed to generate base MAC from mmc")
end

-- MAC 地址加法
local function macaddr_add(mac, inc)
    local bytes = {}
    for byte in string.gmatch(mac, "%x%x") do
        table.insert(bytes, tonumber(byte, 16))
    end
    local num = (bytes[1] << 40) | (bytes[2] << 32) | (bytes[3] << 24) |
...
-- 获取有效基 MAC（优先从 Factory 读取，若无效则回退到 mmc 生成）
-- 参数: offset - Factory 偏移量, fallback_dev_idx - 若提供则回退时加上 (fallback_dev_idx+1)*2 偏移
local function get_valid_mac(offset, fallback_dev_idx)
    local mac = get_mac_from_factory(offset)
    if not is_mac_invalid(mac) then
        return mac
    end
    -- 回退: 使用 mmc 生成的基 MAC
    local base = get_mmc_base_mac()
    if fallback_dev_idx then
        -- 根据 dev_idx 加偏移: dev_idx=1 -> +2, dev_idx=2 -> +4, dev_idx=3 -> +6
        return macaddr_add(base, (fallback_dev_idx + 1) * 2)
    else
        return base
    end
end

function mtkdat.bit_lshift(x, n)
    return x * (2 ^ n)
end

function mtkdat.generate_mld_addr(mld_group_id)
    if mld_group_id < 1 or mld_group_id > 32 then
        error("mld_group_id must be between 1 and 32")
    end

    local base_mac = get_valid_mac(0x4, nil)
    local mac_bytes = {}
    for byte in string.gmatch(base_mac, "%x%x") do
        table.insert(mac_bytes, byte)
    end

    mac_bytes[1] = string.format("%02x", (tonumber(mac_bytes[1], 16) + 0x20) & 0xFF)
    local shift = mtkdat.bit_lshift(mld_group_id - 1, 4)
    mac_bytes[4] = string.format("%02x", (tonumber(mac_bytes[4], 16) + shift) & 0xFF)

    return table.concat(mac_bytes, ":")
end

function mtkdat.generate_bssid(dev_idx, iface_idx)
    if dev_idx < 1 or iface_idx < 1 then
        error("dev_idx and iface_idx must be greater than or equal to 1")
    end

    local offset
    if dev_idx == 1 then
        offset = 0x04
    elseif dev_idx == 2 then
        offset = 0x0a
    elseif dev_idx == 3 then
        offset = 0x2c0
    else
        error("Unsupported dev_idx: " .. dev_idx)
    end

    local base_mac = get_valid_mac(offset, dev_idx)  -- 回退时自动加 (dev_idx+1)*2
    ...
    local first_byte = (tonumber(mac_bytes[1], 16) + 0x02) & 0xFF
    mac_bytes[1] = string.format("%02x", first_byte)
    local shift = mtkdat.bit_lshift(iface_idx - 1, 4)
    local fourth_byte = (tonumber(mac_bytes[4], 16) + shift) & 0xFF
    mac_bytes[4] = string.format("%02x", fourth_byte)
...
local function uci_apply_wireless_mlo_vif(uci_cfg, uci_mld, ap_id_table, sta_id_table)
    ...
    if uci_mld.mld_addr == nil then
        uci_mld.mld_addr = mtkdat.generate_mld_addr(tonumber(uci_mld.mldgroup))
    end
...
function mtkdat.uci_decode_wireless()
    ...
    local index = 1
    for idx, uci_dev in pairs(t["wifi-device"]) do
        uci_dev.idx = index
        index = index + 1
    end
    ...
    local dev = ""
    index = 1
    for idx, uci_vif in pairs(t["wifi-iface"]) do
        if uci_vif.device ~= dev then
            index = 1
            dev = uci_vif.device
        end
        uci_vif.idx = index
        index = index + 1
        uci_vif.ifname = uci_vif[".name"]
        local uci_dev = mtkdat.get_uci_dev_by_dev_name(t, uci_vif.device)
        uci_vif.bssid = mtkdat.generate_bssid(uci_dev.idx, idx + 1)
        uci_vif.mld_link_id = uci_dev.idx
    end
...
function load_l1_profile(path)
    ...
    for i = 1, table.getn(l1_profiles) do
        if l1_profiles[i].profile_path then
            band_profile_path, MacAddress, E2pAccessMode, TestModeEn, WHNAT = load_band_profile(l1_profiles[i]["profile_path"])
...
--        dev.bw = "20"
    ...
    if dev.band == "5G" or dev.band == "5g" then
        dev.acs_skiplist = "52;56;60;64;100;104;108;112;116;120;124;128;132;136;140;144;149;153;157;161;165;169;173;177"
...
local function getDefaultACSConfig(dev)
    if type(dev) ~= "string" or dev == "" then
        return {}
    end

    local cmd = string.format("mwctl %s show acsinfo=stdout", dev)
    local handle = io.popen(cmd)
    if not handle then
        return {}
    end
...
local function dev2cfg(dev, cfg, devname)
    ...
    if dev.band == "2g" then
        cfg.CountryRegion = country_region_map[cfg.CountryCode][1]
    else
        cfg.CountryRegionABand = country_region_map[cfg.CountryCode][2]
    end
...
function mtkdat.uci2dat(uci)
    ...
    for _, dev in pairs(uci["wifi-device"]) do
        if dev.type ~= "mtwifi" then return end
