...
function format_wifirate(rate) {
	...
	if (rate?.he) s += [
		...
		rate?.he_gi != null && `, HE-GI\xa0${rate?.he_gi}`,
	...
	poll_status: function(map, data) {
		...
		radios.forEach(zone => {
			...
			let row = [
				...
				E('span', {}, (function() {
					...
					if (bss.mlo_enable && bss.mld_mac) {
						children.push(E('br'));
						let text = _('MLD: %s').format(bss.mld_mac);
						if (bss.mlo_num_links)
							text += ' ' + _('(%d links)').format(bss.mlo_num_links);
						children.push(E('small', text));
					}
					return children;
				})()),
				hint,
				...
				E('span', {}, (function() {
					if (bss.mlo_enable && bss.mlo_links) {
						...
						bss.mlo_links.forEach(function(link, idx) {
							const lid = (link.link_id != null) ? link.link_id : idx;
							const elements = [
								E('strong', _('Link %d').format(lid + 1)),
								E('br'),
			...
			if (bss.network.isClientDisconnectSupported()) {
				...
				const disconnectClient = function(ifname, mac) {
					//  1: mwctl 2: iwpriv
	...
	renderOverview: function(zones) {
		...
		s.addModalOptions = function(s) {
			return network.getWifiNetwork(s.section).then(function(radioNet) {
				...
				const band = uci.get('wireless', radioNet.getWifiDeviceName(), 'band');
				...
				if (hwtype == 'mtwifi') {
					...
					o = ss.taboption('advanced', form.Flag, 'ldpc', _('Tx LDPC'), _('Low-Density Parity-Check'));
					o.default = o.enabled;
					o.rmempty = false;
					...
					const is_dbdc_main = uci.get('wireless', radioNet.getWifiDeviceName(), 'dbdc_main') == '1';
				...
				if (hwtype == 'mtwifi') {
					...
					o = ss.taboption('general', form.ListValue, 'multi_ap', _('Enable Multi-AP functionality'));
					o.depends('mode', 'ap');
					o.depends('mode', 'sta');
					o.depends('mode', 'ap-wds');
					o.depends('mode', 'mesh');
					...
					o.value('3', _('Both Backhaul BSS and Fronthaul BSS'));
					o.default = '0';
					...
					o = ss.taboption('advanced', form.ListValue, 'wdsphymode', _('WDS PHY Mode'), _('If GREENFIELD seems to be unstable,try to use OFDM instead.HE/EHT is only available for 11AX/11BE devices.'));
					o.depends('mode', 'ap-wds');
					o.depends('mode', 'sta-wds');
					o.value('CCK', _('CCK'));
					o.value('OFDM', _('OFDM'));
					o.value('HTMIX', _('HTMIX'));
					...
					o = ss.taboption('advanced', form.Flag, 'hairpin', _('Wireless Hairpin'), _('It is primarily designed to address the issue of inability to communicate directly between clients within the same WiFi network.'));
					add_dependency_permutations(o, { isolate: ['0'], mode: ['ap', 'ap-wds'] });
					o.default = '1';
				...
				o.write = function(section_id, value) {
					...
					if (co.isActive(section_id) && e && (c == 'tkip' || c == 'ccmp' || c == 'ccmp256' || c == 'gcmp' || c == 'gcmp256' || c == 'ccmp+gcmp256' || c == 'tkip+ccmp'))
						e += '+' + c;
				...
				o.value('ccmp+gcmp256', _('Force CCMP/GCMP-256 (AES)'));
				...
				o.cfgvalue = function(section_id) {
					...
					if (v.match(/\+/)) {
						...
						if (v == 'aes')
							v = 'ccmp';
						else if (v == 'aes+gcmp256' || v == 'ccmp+gcmp256')
							v = 'ccmp+gcmp256';
				...
				add_dependency_permutations(o, { mode: ['ap'], encryption: ['psk', 'psk2', 'psk+psk2', 'psk-mixed'] }); */

				o = ss.taboption('encryption', form.Value, 'ownip', _('Configure AP its own IP address'), _('own_ip_addr is global setting ,in this point, let all mbss set the same ip addr for safe.'));
				...
				if (hwtype == 'mtwifi') {
					...
					//if (has_80211r)
						add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['psk2', 'psk-mixed', 'sae', 'sae-mixed', 'sae-ext', 'sae_sae-ext', 'sae-ext-mixed'] });
					o.rmempty = true;
					...
					// add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['psk', 'psk2', 'psk-mixed', 'sae', 'sae-mixed', 'sae-ext', 'sae_sae-ext', 'sae-ext-mixed'] });
					o.depends('mode', 'ap');
					o.depends('mode', 'ap-wds');
					...
					o = ss.taboption('roaming', form.Flag, 'wnm_notify', _('WNMNotify'), _('802.11v: Enable WNM notification.'));
				...
				if (hwtype == 'mtwifi') {
					...
					add_dependency_permutations(o, { mode: ['ap', 'ap-wds'], encryption: ['psk2', 'psk-mixed', 'sae', 'sae-mixed', 'sae-ext', 'sae_sae-ext', 'sae-ext-mixed', 'wpa2', 'wpa3', 'wpa3-mixed'] }); */
		...
		s.handleRemove = function(section_id, radioNet, ev) {
			...
			const sectionMode = uci.get('wireless', section_id, 'mode');
			...
			return form.TypedSection.prototype.handleRemove.apply(this, [section_id, ev]).then(() => {
				if (sectionMode === 'sta') {
					uci.set('wireless', radioName, 'channel', 'auto');

					/*return fs.exec('/sbin/wifi', ['reload', radioName]).catch(function(e) {
		...
		s.handleAdd = function(radioDev, ev) {
			const hwtype = radioDev.get('type');

			if (hwtype == 'mtwifi')
			{
				const wifi_sections = uci.sections('wireless', 'wifi-iface');
				let mbssid_num = 0;
				let max_mbssid_num = 16;
